...
class Extraction extends StatefulWidget {
  const Extraction({super.key, required this.user});

  final User user;

  @override
  State<StatefulWidget> createState() => _ExtractionState();
}
...
class _ExtractionState extends State<Extraction> {
  ...
  //user object for changing it's point and extIndex
  late User user;
  ...
  Future<void> getQuestions() async {
    ...
    final snapshot = await db
        .collection("extQuestions")
        .where("id", isGreaterThanOrEqualTo: user.extIndex + 1)
        .where("id", isLessThanOrEqualTo: user.extIndex + 5)
        .get();
  ...
  Widget build(BuildContext context) {
    return !questionsExist
        ? FinishedPage(questionType: "extraction", user: user)
        : Scaffold(
